...
class Closeup : ApplicationAdapter() {
    ...
    override fun render() {
		...
		with(renderer) {
			...
			// Draw the star
			color = Color.YELLOW
			circle(STAR_CENTER_X, STAR_CENTER_Y, STAR_RADIUS, SEGMENTS)

			// Draw the planet
			color = Color.GREEN
			circle(PLANET_CENTER_X, PLANET_CENTER_Y, PLANET_RADIUS, SEGMENTS)

			// Draw the satellite
			color = Color.RED

			val halfSize = SATELLITE_SIZE / 2
			rect(SATELLITE_POSITION_X - halfSize, SATELLITE_POSITION_Y - halfSize,
					halfSize, halfSize,
    ...
    private fun trackStar(camera: OrthographicCamera, aspectRatio: Float) {
		with (camera) {
			viewportHeight = 2 * STAR_RADIUS
			// This will stretch the star, since the aspect ratio doesn't match
			//        viewportWidth = 2 * STAR_RADIUS;
			viewportWidth = aspectRatio * viewportHeight
			position.set(STAR_CENTER_X, STAR_CENTER_Y, 0f)
			translate(STAR_RADIUS * (aspectRatio - 1), 0f)
		}
    }
    ...
    private fun trackPlanet(camera: OrthographicCamera, aspectRatio: Float) {
		with(camera) {
			viewportHeight = 2 * PLANET_RADIUS
			viewportWidth = aspectRatio * viewportHeight
			position.set(0f, 0f, 0f)
			translate(PLANET_CENTER_X, PLANET_CENTER_Y, 0f)
		}
    }
    ...
    private fun trackSatellite(camera: OrthographicCamera, aspectRatio: Float) {
		with(camera) {
			viewportHeight = 2 * SATELLITE_SIZE
			viewportWidth = aspectRatio * viewportHeight
			position.set(SATELLITE_POSITION_X, SATELLITE_POSITION_Y, 0f)
			rotate(SATELLITE_ROTATION)
		}
    }
